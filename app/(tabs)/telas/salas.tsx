...
const SalasScreen: React.FC<SalasScreenProps> = ({ onNavigate }) => {
  ...
  const getSalas = async () => {
    try {
      ...
      if (userType === 'Coordenador') {
        const response = await axios.get('http://192.168.0.97:8000/api/salas');
        setSalas(response.data);
      } else if (userType === 'Professor') {
        const storedUser = await AsyncStorage.getItem('user');

        if (storedUser) {
          const response = await axios.post('http://192.168.0.97:8000/api/get_user_room/', {
            username: storedUser
          });
  ...
  const handleAddRoom = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.0.97:8000/api/add_sala/', newRoom);
