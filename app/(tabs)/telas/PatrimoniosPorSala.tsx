...
const PatrimoniosPorSala: React.FC<{ route: any; onNavigate: (screen: string) => void }> = ({ route, onNavigate }) => {
  ...
  useEffect(() => {
    const fetchPatrimonios = async () => {
      try {
        const response = await axios.get(`http://192.168.0.97:8000/api/inventarios/`);
  ...
  const deleteSala = async () => {
    try {
      await axios.delete(`http://192.168.0.97:8000/api/delete_sala`, { data: { sala: salaNome } });
  ...
  const handleEdit = async () => {
    try {
      const response = await axios.put(`http://192.168.0.97:8000/api/editar_sala`, salaData);
